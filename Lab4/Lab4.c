...
double getNum(int bf, int minusMem, double degree, double numb, char* str, int dotMem, int minus) {
...
int main() {
    ...
    int i = 0, minusMem = 0, mem = 0, dot = 0, dotMem = 0, amount = 0, flag = 0, word = 0, intNumb = 0, minus = 0;
    ...
    while (a[i] != 0) {
        ...
        if (word == 1 && minus == 1) numb *= -1;
        if (minus == 0 && word == 0 && checkS(a[i]) == 0) {
            i++;
            goto exit;
        }
        ...
        if ((dot < 2 && mem > 0) || word == 1) {
            if (dot == 0 && word == 0)
                numb = getNum(mem, minusMem, degree, numb,&a[0], -1, minus);

            if (dot == 1 && word == 0) {
                if (mem - dotMem > 5) mem = dotMem + 5;
                for (int d = 0; d < mem - dotMem; d++)  degree /= 10;
                numb = getNum(mem, minusMem, degree, numb, a, dotMem, minus);
            }
